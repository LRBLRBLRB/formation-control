...
class magui:
        ...
        for i in range(size[0]):
            for j in range(size[1]):
                if self.graph.adjmatrix[i,j]==1:
                    xsegs.append([self.graph.agents[i].coordinates.x,self.graph.agents[j].coordinates.x])
                    ysegs.append([self.graph.agents[i].coordinates.y,self.graph.agents[j].coordinates.y])
    ...
    def animate(self):
        ...
        ani=animation.FuncAnimation(self.fig,self.updateAnimation,frames=range(0,1000),init_func=self.initAnimation,interval=500,blit=True)
        plt.show()
